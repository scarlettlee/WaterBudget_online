...
def compute_newR1(row, l, col): # E_P# and PR_####_P_r
    ...
    if pd.isnull(b) or b == -9999.0:    
        ...
        row[col+'1'] = b - sign(b)*(1+overAdjustPerc)*a # PR_####_P_r1 = PR_####_P_r - 1.2*Ep
        # if it is precipitation, reverse its sign
        if col[:-2].endswith('P'):
            row[col+'1'] = -row[col+'1']
            row[col[:-2]+'_1'] = row[col[:-2]] + row[col+'1'] # PR_####_P_2 = PR_####_P + PR_####_P_r1
        else:
            row[col[:-2]+'_1'] = row[col[:-2]] - row[col+'1'] # PR_####_E_2 = PR_####_E - PR_####_E_r1
    else:
        row[col+'1'] = 0
...
def compute_newR2(row, l, col): # P and PR_####_P  
    a = row[l] # the reference to be compared with P
    b = row[col+'_2']   # the PR_####_P_2
    ...
    # Check if a is negative and b is positive
        ...
        # print("negative values",l,col,a,b)
        row[col+'_r2'] = b-a
        # if it is precipitation, reverse its sign
        if col.endswith('P'):
            row[col+'_r2'] = -row[col+'_r2']
        row[col+'_2'] = a
...
def redistributeR(row,filtered, index):
    # get all columns with r1： PR_####_P_r1/r2
...
for fl in fileList:
    ...
    # Compute r1 and redistribute errors (extremes)
            ...
            for col in filtered:#['MSD_5414_P']:#              
                ...
                if col_number:
                    ...
                    if digit is not None:
                        col_e = f'E_{l}{digit}'
                        data = data.apply(lambda row: compute_newR1(row, col_e, col + '_r'), axis=1)  # E_P#, and PR_####_P_r
    # print(data.columns.to_list())
    ...
    ############################################################
    # compute r2 based on r1
    for combin in Combinations:
        for l in lab:
            for m in met:
                data[m + '_' + combin + '_' + l + '_2'] = data[m + '_' + combin + '_' + l + '_1'] 
    ############################################################
    ...
    # Compute r2 and redistribute errors (negatives)
    for l in lab:        #['P', 'E', 'R', 'S']['P']:#
        for m in met:    #['PR', 'CKF', 'MCL', 'MSD'] ['MSD']:#   
            r = re.compile(m+"_\d{4}_"+l+"$") # PR_####_P
            filtered = list(filter(r.match, columns))
            ...
            for col in filtered:#['MSD_5414_P']:#
                ...
                data = data.apply(lambda row: compute_newR2(row, l, col), axis=1)  # P and PR_####_P  +l

    # # redistribute r2 for each BCC and composition
    # for combin in Combinations: 
    #     for m in met: # PR_####
    #         r = re.compile(m+"_"+combin+"_[PERS]") # PR_1111_P/E/R/S + null/_w/_r/_r1/r2/_1 (24 columns in total)
    #         filtered = list(filter(r.match, columns))
