...
for fl in fileList:
    ...
    data['E_P1'] = abs(data['P1']-data['P'])
    data['E_P2'] = abs(data['P2']-data['P'])
    data['E_P3'] = abs(data['P3']-data['P'])
    data['E_P4'] = abs(data['P4']-data['P'])
    data['E_P5'] = abs(data['P5']-data['P'])
    # For E: E#*20%
    data['E_E1'] = abs(data['E1']-data['E'])
    data['E_E2'] = abs(data['E1']-data['E'])
    data['E_E3'] = abs(data['E1']-data['E'])
    data['E_E4'] = abs(data['E1']-data['E'])
    # For R: R*7%
    data['E_R1'] = data['R1']*0.07
    # For S: true value TWSC_GRACE_Mascon_JPL_calculate
    data['E_S1'] = abs(data['S1']-data['S'])
    data['E_S2'] = abs(data['S2']-data['S'])
    data['E_S3'] = abs(data['S3']-data['S'])
    data['E_S4'] = abs(data['S4']-data['S'])
    ...
    if test:
        data.to_csv(outPath+fn+"_test2.csv",index=False)
    else:
